...
import org.opencron.common.util.atomic.AtomicUpdater;
...
public class NettyChannelGroup implements ChannelGroup {
    ...
    private static long LOSS_INTERVAL = SystemPropertyUtil
            .getLong("opencron.io.channel.group.loss.interval.millis", TimeUnit.MINUTES.toMillis(5));
    ...
    private final ConcurrentMap<String, Integer> weights = ContainerUtils.newConcurrentMap(true);
    ...
    public List<? extends Channel> channels() {
        return ContainerUtils.newArrayList(channels);
    }
    ...
    public void setWeight(Directory directory, int weight) {
        ...
        weights.put(directory.directory(), weight > Constants.MAX_WEIGHT ? Constants.MAX_WEIGHT : weight);
    }
